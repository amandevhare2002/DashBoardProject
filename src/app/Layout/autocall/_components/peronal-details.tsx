...
const PersonalDetails = forwardRef(
  (
    ...
    const handleUndo = useCallback(() => {
      ...
      // Update updatedPersonalDetails
      setUpdatedPersonalDetails(previousState);
    ...
    useEffect(() => {
      ...
      if (foundIndex !== -1 && foundIndex !== currentDataIndex) {
        ...
        console.log(
          " No matching Data object found for current personalDetails",
        );
      }
    ...
    const moveBox = useCallback(
      (id: string, left: number, top: number) => {
        ...
        // ✅ UNDO: Check if position actually changed before saving to history
        const currentRownum = isPDFPreviewOpen ? field.PDFRownum : field.Rownum;
        const currentColnum = isPDFPreviewOpen ? field.PDFColnum : field.Colnum;

        if (
          currentRownum !== constrainedLeft ||
          currentColnum !== constrainedTop
        ) {
          // Position changed, save DEEP COPY of current state to history
          const deepCopyState = JSON.parse(
            JSON.stringify(updatedPersonalDetails),
          );
          setUndoHistory((prev) => [...prev, deepCopyState]);
        } else {
    ...
    const AddDropDownData = async (
      ...
      try {
        const columnsData = updatedPersonalDetails[value].Values.find(
          (reportCol: any) =>
            Number(reportCol.FieldID) === Number(columnSelect),
        );
        if (!columnsData) {
          return [];
        }
            ...
            tab.Values.forEach((field: any) => {
              if (
                ...
                if (
                  ...
                  foundFieldValue =
                    field.addmorevalues[addMoreIndex].FieldValue;
                } else {
                  foundFieldValue =
        ...
        for (const parent of field.parentFields) {
          ...
          for (const config of field.apifields) {
            if (config.ArrayName === "parentFields") {
              const val =
                parent[config.KeyValue] ?? field[config.KeyValue] ?? "";
    ...
    const handleExcelFileUpload = async (field: any) => {
          ...
          fieldGroup?.Values?.forEach((res: any) => {
            field.buttonFields.forEach((response: any) => {
              if (
                ...
                let fieldValue = saveData[res.FieldName];
                if (res.FieldType === "UPLOAD" && fieldValue?.base64string) {
                  fieldValue = fieldValue.base64string;
                }
          ...
          if (fieldDef) {
            ...
            if (fieldDef.FieldType === "UPLOAD" && fieldValue?.base64string) {
              ...
              payload.Base64string =
                fieldValue.base64string.split(",")[1] || "";
              fileFound = true;
            }
      ...
      // Fallback to direct field value if no buttonFields
        ...
        payload.Base64string =
          field.FieldValue.base64string.split(",")[1] || "";
      } else {
        toast.error("No file selected for upload!", { style: { top: 80 } });
        return;
      }
    ...
    const updateCardFieldData = (
      ...
      const widthValue =
        parseInt(fieldWidth.toString().replace("px", "")) || 100;
      const heightValue =
    ...
    const getCardFields = (field: any, cardField: any) => {
      return updatedPersonalDetails[value]?.Values.map(
        (res: any, i: number) => {
    ...
    const onResize = (e: any, direction: any, ref: any, d: any, field: any) => {
      ...
      // ✅ UNDO: Save DEEP COPY of current state to history BEFORE making any changes
      const deepCopyState = JSON.parse(JSON.stringify(updatedPersonalDetails));
      setUndoHistory((prev) => [...prev, deepCopyState]);
    ...
    const columns =
      ...
      updatedPersonalDetails[value]?.ds?.length > 0
        ? Object.keys(updatedPersonalDetails[value]?.ds[0]).map(
            (column: any) => {
              const isKeyColumn =
                column.toLowerCase() ===
                (updatedPersonalDetails &&
    ...
    const showSubmit =
      ...
      values.some(
        (val: any) => val.ValueType === "SUBMIT" && val.DefaultVisible,
      );
    ...
    const tableModuleID =
      updatedPersonalDetails[value]?.Values?.find(
        (field: any) => field?.ModuleID,
      )?.ModuleID ||
      menuIDQuery ||
      menuID;
    ...
    useEffect(() => {
      ...
      const defaultTabIndex = personalDetails.findIndex(
        (commissionTab: any) => {
          const tabAliasName =
            commissionTab?.TabAliasName?.trim().toLowerCase();
          ...
          if (currentTabIndex !== -1) {
            ...
            if (result.data.Table && Array.isArray(result.data.Table)) {
              ...
              const tableField = newUpdatedDetails[currentTabIndex].Values.find(
                (f: any) =>
                  f.FieldID == tableFieldID && f.FieldType === "TABLE",
              );
            ...
            if (result.data.ChartData && result.data.Chartids) {
              ...
              chartIds.forEach((chartInfo: any, index: number) => {
                const chartFieldID = chartInfo.FieldID;
                const chartField = newUpdatedDetails[
                  currentTabIndex
                ].Values.find(
                  (f: any) =>
                ...
                newUpdatedDetails.forEach((responseData: any) => {
                  responseData?.Values?.forEach((formField: any) => {
                    if (
                      ...
                      updatedSaveData[formField.FieldName] =
                        responseField.Value;
    ...
    const handleSubmitDirectlyInPersonalDetails = async () => {
      ...
      try {
        ...
        const addField = (
          ...
          console.log(
            `✅ Added regular field: ${fld.FieldName} = ${fieldValue}`,
          );
        };
        ...
        if (isUpdateTabWiseFlag) {
          ...
          console.log(
            "📋 Collecting ALL tabs fields (IsUpdateTabWise = false)",
          );
      ...
      <>
        ...
        <Modal isOpen={formulaModal}>
          ...
          <ModalFooter>
            ...
            <Button onClick={() => setFormulaModal(!formulaModal)}>
              Cancel
            </Button>
          </ModalFooter>
        </Modal>
        ...
        {!edit ? (
          ...
          >
            {!!isAccordian ? (
              ...
              <>
                {updatedPersonalDetails &&
                  updatedPersonalDetails.length > 1 && (
                            ...
                            styleProps={{
                              ...
                              nestdtabsecbgcolor:
                                information?.Data?.[0]?.Fields?.[0]
                                  ?.nestdtabsecbgcolor,
                              Isbold:
                                information?.Data?.[0]?.Fields?.[0]?.Isbold,
                              IsItalic:
                              ...
                              clsicon:
                                information?.Data?.[0]?.Fields?.[0]?.clsicon,
                              clscolor:
                  ...
                  {updatedPersonalDetails?.[value]?.IsAddMore ? (
                    <div
                      ...
                      style={{
                        height: `${information.Data[0]?.PageHeight}vh`,
                        backgroundColor:
                          updatedPersonalDetails?.[value]?.Bgcolor,
                      }}
                    >
                      {Object.entries(groupedFields).map(
                        ([groupName, fieldsInGroup]: any) => {
                    ...
                    >
                      <RenderFields
                        ...
                        handleSaveData={handleSaveData}
                        parentTabVisible={
                          updatedPersonalDetails[value]?.DefaultVisible !==
                          false
                        }
                        tableBtnInfo={tableBtnInfo}
                        mandatoryFieldErrors={mandatoryFieldErrors}
