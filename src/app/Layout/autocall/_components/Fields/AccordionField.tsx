...
const Accordionfield = ({
  ...
  const handleChangeNestedAccordian =
    (panel: number) => (event: React.SyntheticEvent, isExpanded: boolean) => {
      if (panel === tabValue) {
        setTabValue(-1);
      } else {
  ...
  const isAccordian = field.FieldType === "ACCORDION" || false;
                  ...
                  >
                    {nestedTabs.map((nestedTab: any, index: number) => (
                      <Accordion
                        ...
                        onChange={handleChangeNestedAccordian(index)}
                        key={index}
                        disableGutters
                        square
                        sx={{
                            ...
                            <div className="w-full flex flex-col gap-3">
                              {Object.entries(groupedFields).map(
                                ([groupName, fieldsInGroup]: any) => {
                                  return fieldsInGroup?.[0]?.addmorevalues
                                    ?.length > 0 ? (
                                    <div key={groupName}>
                                      <h4
                                        style={{
